...
app.get("/search/:result/", validate, async (req, res) => {
	...
	let songsResult = await db.collection("songs").find({$or: [{title: new RegExp(`^${result}`, "mi")}, {username: new RegExp(`^${result}`, "mi")}]}).toArray();
    let artistResult = await db.collection("users").find({username: new RegExp(`^${result}`, "mi")}).toArray();
    let artistName = "";
